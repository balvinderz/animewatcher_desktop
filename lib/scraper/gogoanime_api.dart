...
class GogoAnimeScraper
{
  ...
  Future<List<Anime>> searchAnime(String text) async
  {
    ...
    Response response = await _dio.get(_gogoAnimeUrl+"/search.html?keyword=$text");
    var searchPage  = parse(response.data);

    List<dom.Element> lis = searchPage.querySelector("div.main_body").querySelector("div.last_episodes").querySelector("ul.items]").querySelectorAll("li");
    for(dom.Element li in lis)
      {
        String link = _gogoAnimeUrl + li.querySelector("div.img > a").attributes['href'];

        String name = li.querySelector("div.img > a").attributes['title'];

         String imageLink = li.querySelector("div.img >a > img").attributes['src'];

        Anime anime = Anime(link : link,name:name ,episodeNo : "",imageLink: imageLink);
  ...
  Future<AnimeDetails> getAnimeDetails(String baseUrl) async
  {
    ...
    List<dom.Element> lis = animePage.querySelectorAll("div.anime_video_body > ul#episode_page > li");
    String summary  = animePage.querySelectorAll("div.anime_info_body_bg >p.type")[1].text;
    print(summary);
    return AnimeDetails.fromJson({
      'description' : summary,
      'start_episode' : int.parse(lis[0].querySelector("a").attributes['ep_start'])+1,
      'end_episode' :  int.parse(lis[lis.length-1].querySelector("a").attributes['ep_end'])
    });
